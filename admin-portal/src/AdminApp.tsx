...
async function apiRequest<T>(path: string, token: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    let code = response.status === 401 ? "session_expired" : response.status === 403 ? "admin_required" : "request_failed";
    try {
...
function Login({ onAuthenticated }: { onAuthenticated: (token: string) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const result = (await login.json()) as { accessToken: string; role: string };
      ...
      const message = reason instanceof Error ? reason.message : "request_failed";
      setError(
    ...
    try {
      if (view === "overview") {
        const [overview, providerData] = await Promise.all([
          apiRequest<{ metrics: Metrics }>(`/api/admin/overview?periodDays=${period}`, token),
        ...
        const data = await apiRequest<{ users: UserRecord[] }>(
          `/api/admin/users?query=${encodeURIComponent(search)}&pageSize=50`, token,
        );
        setUsers(data.users);
      } else if (view === "providers") {
        const data = await apiRequest<{ providers: Provider[] }>("/api/admin/providers", token);
        setProviders(data.providers);
      } else {
        const data = await apiRequest<{ events: AuditEvent[] }>("/api/admin/audit-events?pageSize=50", token);
        setEvents(data.events);
      }
    } catch (reason) {
      const message = reason instanceof Error ? reason.message : "request_failed";
      if (message === "session_expired" || message === "admin_required") signOut();
      else setNotice("The admin service is unavailable. Check the backend connection and try again.");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><div className="brand-mark small">C</div><div><strong>Copilot</strong><span>ADMIN CONSOLE</span></div></div>
        <nav aria-label="Admin navigation">
          {(Object.keys(icons) as View[]).map((item) => (
            <button key={item} className={view === item ? "active" : ""} onClick={() => setView(item)}>
              <span>{icons[item]}</span>{item === "audit" ? "Audit trail" : item}
            </button>
          ))}
        </nav>
        <div className="sidebar-foot">
          <div className="secure-dot"><i />Backend verified</div>
      ...
      <main className="content">
        ...
        {view === "overview" && (
          <section className="page-stack">
            <div className="section-heading"><div><h2>System overview</h2><p>A privacy-safe snapshot of your Copilot workspace.</p></div><select value={period} onChange={(e) => setPeriod(Number(e.target.value))}><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option><option value={90}>Last 90 days</option></select></div>
        ...
        {view === "providers" && (
          <section className="page-stack">
            <div className="section-heading">
              <div><h2>AI providers</h2><p>Validate, rotate, and roll back backend-only credentials.</p></div>
            </div>
            <div className="provider-grid">
              {providers.map((provider) => (
                <article className="provider-card" key={provider.provider}>
                  <div className={`provider-logo ${provider.provider}`}>{provider.provider === "openai" ? "O" : "G"}</div>
                  <div className="provider-card-head">
                    <div><p>{provider.purpose.toUpperCase()}</p><h3>{provider.provider === "openai" ? "OpenAI" : "Groq"}</h3></div>
                    <span className={`status ${provider.status}`}>{provider.status}</span>
                  </div>
                  <div className="provider-details provider-details-wide">
                    <div><span>Credential source</span><strong>{provider.source}</strong></div>
                    <div><span>Model</span><strong>{provider.model ?? "Not selected"}</strong></div>
                    <div><span>Credential</span><strong>{provider.maskedHint ?? "Hidden"}</strong></div>
                    <div><span>Last validated</span><strong>{provider.lastValidatedAt ? formatDate(provider.lastValidatedAt) : "Not validated"}</strong></div>
                  </div>
                  {!provider.managementAvailable && <p className="configuration-warning">Set COPILOT_CREDENTIAL_MASTER_KEY and restart the backend to enable management.</p>}
                  <div className="provider-actions">
                    <button className="secondary-button" disabled={!provider.managementAvailable} onClick={() => setCredentialDialog({ provider, mode: "replace" })}>Validate & replace</button>
                    <button className="ghost-button" disabled={!provider.managementAvailable || !provider.canRollback} onClick={() => setCredentialDialog({ provider, mode: "rollback" })}>Roll back</button>
                  </div>
                  <p className="provider-footnote">Existing keys remain unreadable. New values are sent once over the protected backend API.</p>
                </article>
              ))}
            </div>
            <div className="info-banner"><span>i</span><div><strong>Runtime reload enabled</strong><p>A successful replacement is used by new operations immediately. In-progress operations finish with their existing client.</p></div></div>
          </section>
        )}
        {view === "audit" && (
...
function credentialErrorMessage(code: string): string {
  const messages: Record<string, string> = {
    credential_master_key_not_configured: "Credential management is not configured on the backend.",
    ...
    provider_model_not_available: "This key does not have access to the selected model.",
    provider_rate_limited: "The provider rate-limited validation. Try again shortly.",
    provider_timeout: "The provider did not respond in time.",
    provider_unavailable: "The provider is temporarily unavailable.",
    recent_authentication_failed: "Your administrator session is no longer authorized.",
    rollback_credential_not_found: "There is no previous managed key to restore.",
  };
...
function CredentialDialog({
  ...
  const [busy, setBusy] = useState(false);
  const providerName = provider.provider === "openai" ? "OpenAI" : "Groq";

  async function validateCredential() {
    setBusy(true);
    setMessage("");
    try {
      const result = await apiRequest<{ valid: boolean; errorCode: string | null }>(
        `/api/admin/providers/${provider.provider}/validate`,
        token,
        {
          method: "POST",
      ...
      if (result.valid) {
        ...
        setMessage(credentialErrorMessage(result.errorCode ?? "request_failed"));
      }
    } catch (reason) {
      setValidation("invalid");
      setMessage(credentialErrorMessage(reason instanceof Error ? reason.message : "request_failed"));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function replaceCredential() {
    ...
    setMessage("");
    try {
      await apiRequest(`/api/admin/providers/${provider.provider}/credential`, token, {
        method: "PUT",
        body: JSON.stringify({ credential, model: model || null }),
      });
      setCredential("");
  ...
  async function rollbackCredential() {
    ...
    setMessage("");
    try {
      await apiRequest(`/api/admin/providers/${provider.provider}/rollback`, token, {
        method: "POST",
        body: JSON.stringify({}),
      });
      setCurrentPassword("");
      await onCompleted(`${providerName} restored to the previous validated credential.`);
    } catch (reason) {
      setMessage(credentialErrorMessage(reason instanceof Error ? reason.message : "request_failed"));
    } finally {
      setBusy(false);
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="credential-modal" role="dialog" aria-modal="true" aria-labelledby="credential-title">
        <div className="modal-head">
          <div><p className="eyebrow">{mode === "replace" ? "SECURE ROTATION" : "SAFE ROLLBACK"}</p><h2 id="credential-title">{mode === "replace" ? `Manage ${providerName}` : `Roll back ${providerName}`}</h2></div>
          <button className="modal-close" onClick={onClose} aria-label="Close">x</button>
        </div>
        {mode === "replace" ? (
          <div className="modal-form">
            <label>Model<input value={model} onChange={(event) => { setModel(event.target.value); setValidation("idle"); }} placeholder={provider.purpose === "llm" ? "Enter language model ID" : "Enter transcription model ID"} required /></label>
            <label>New API key<span className="password-field"><input type={showCredential ? "text" : "password"} value={credential} onChange={(event) => { setCredential(event.target.value); setValidation("idle"); }} placeholder="Paste the new provider key" autoComplete="off" required /><button type="button" className="password-toggle" onClick={() => setShowCredential((visible) => !visible)} aria-label={showCredential ? "Hide API key" : "Show API key"}>{showCredential ? "Hide" : "Show"}</button></span></label>

            {message && <div className={validation === "valid" ? "validation-message valid" : "validation-message invalid"}>{message}</div>}
            <div className="modal-actions"><button className="ghost-button" onClick={onClose}>Cancel</button><button className="secondary-button" disabled={busy || !credential || !model} onClick={() => void validateCredential()}>{busy ? "Working..." : "Validate"}</button><button className="primary-button compact" disabled={busy || validation !== "valid"} onClick={() => void replaceCredential()}>Activate key</button></div>
          </div>
        ) : (
          <div className="modal-form">
            <div className="rollback-warning"><strong>Restore the previous credential?</strong><p>The previous encrypted version will be validated before activation. The current key remains active if validation fails.</p></div>

            {message && <div className="validation-message invalid">{message}</div>}
            <div className="modal-actions"><button className="ghost-button" onClick={onClose}>Cancel</button><button className="primary-button compact" disabled={busy} onClick={() => void rollbackCredential()}>{busy ? "Validating..." : "Validate & roll back"}</button></div>
          </div>
        )}
        <p className="modal-security">Keys are write-only, encrypted before storage, and cleared from this form after use.</p>
      </section>
    </div>
  );
}
